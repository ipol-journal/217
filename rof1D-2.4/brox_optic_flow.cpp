              ...
              )
{
    ...
    du[k] = (1. - w) * du[k] + w * (Au[k] + alpha * div_du) / Du[k];
    dv[k] = 0 * ((1. - w) * dv[k] + w * (Av[k] - D[k] * du[k] + alpha * div_dv) / Dv[k]);
                ...
                )
{
    ...
    for (int index_flow = 0; index_flow < size; index_flow++)
        v[index_flow] = 0;
    ...
    for (int no = 0; no < outer_iter; no++)
    {
        ...
        for (int ni = 0; ni < inner_iter; ni++)
        {
            ...
            while (error > TOL && nsor < MAXITER)
            {
                ...
                #pragma omp parallel for reduction(+:error) num_threads((number_of_threads < (ny-1)) ? number_of_threads : 1)
                for (int i = 1; i < ny - 1; i++)
                {
                    for (int j = 1; j < nx - 1; j++){

                        error += sor_iteration (Au, Av, Du, Dv, D, du, dv, alpha, psi1,
