         ...
         )
{
    ...
    const int corner_down_left = ((ny - 1) * nx);
    
    dx[corner_down_left] = 0.5 * (input[corner_down_left + 1] - input[corner_down_left]);
    dy[corner_down_left] = 0.5 * (input[corner_down_left] - input[(ny - 2) * nx]);

    const int corner_down_right = ny * nx - 1;

    dx[corner_down_right] = 0.5 * (input[corner_down_right] - input[corner_down_right - 1]);
    dy[corner_down_right] = 0.5 * (input[corner_down_right] - input[(ny - 1) * nx - 1]);
