...
xmalloc (size_t size)
{
    ...
    if (!p)
    {
        ...
        fail ("xmalloc: out of memory when requesting " "%zu bytes (%gMB)",	//:\"%s\"",
...
xfopen (const char *s, const char *p)
{
    ...
    if (!s)
        fail ("trying to open a file with NULL name");
...
xfclose (FILE * f)
{
    ...
    if (f != stdout && f != stdin && f != stderr)
    {
        int r = fclose (f);
        if (r)
            fail ("fclose error");	// \"%s\"", strerror(errno));
    }
}

static int
pick_char_for_sure (FILE * f)
{
    int c = getc (f);
    if (EOF == c)
...
static void
eat_spaces_and_comments (FILE * f)
{
...
iio_type_id (size_t sample_size, bool ieeefp_sample, bool signed_sample)
{
    ...
    {
        if (signed_sample)
            fail ("signed floats are a no-no!");
        switch (sample_size)
        {
        case sizeof (float):
...
load_rest_of_file (long *on, FILE * f, void *buf, size_t bufn)
{
    ...
    char *t = xmalloc (ntop);
    if (!t)
        fail ("out of mem (%zu) while loading file", ntop);
    memcpy (t, buf, bufn);
    while (1)
    {
        if (n >= ntop)
        {
            ...
            t = xrealloc (t, ntop);
            if (!t)
                fail ("out of mem (%zu) loading file", ntop);
...
put_data_into_temporary_file (void *filedata, size_t filesize)
{
    ...
    if (cx != 1)
        fail ("fwrite to temporary file failed");
    xfclose (f);
    return filename;
}
...
iio_fmemopen (void *data, size_t size)
{
    ...
    FILE *r = fmemopen (data, size, "r");
    if (!r)
        fail ("fmemopen failed");
    return r;
#else
#ifdef  I_CAN_HAS_FUNOPEN	// BSD case
    fail ("implement fmemopen using funopen here");
#else // portable case
    FILE *f = tmpfile ();
    if (!f)
...
read_beheaded_png (struct iio_image *x, FILE * f, char *header, int nheader)
{
    ...
    if (!pp)
        fail ("png_create_read_struct fail");
    png_infop pi = png_create_info_struct (pp);
    if (!pi)
        fail ("png_create_info_struct fail");
...
read_whole_jpeg (struct iio_image *x, FILE * f)
{
    ...
    FORI (size[1])
    {
        ...
        if (1 != r)
            fail ("failed to rean jpeg scanline %d", i);
    }
                    ...
                    FILE * fin, char *header, int nheader)
{
    ...
    if (r)
        fail ("read whole jpeg returned %d", r);
    fclose (f);
    xfree (filedata);
                    ...
                    FILE * fin, char *header, int nheader)
{
    if (global_variable_containing_the_name_of_the_last_opened_file)
    {
        ...
        if (r)
            fail ("read whole tiff returned %d", r);
        return 0;
    }
...
read_beheaded_qnm (struct iio_image *x, FILE * f, char *header, int nheader)
{
    ...
    if (!isspace (pick_char_for_sure (f)))
        return -6;
...
read_beheaded_rim_ccimage (struct iio_image *x, FILE * f, bool swp)
{
    ...
    if (pm_form == 0x8001)
    {
        ...
        if (r != nsamples)
            fail ("rim reader failed at reading %zu "
                  "samples (got only %zu)\n", (size_t) nsamples, r);
...
read_whole_exr (struct iio_image *x, char *filename)
{
    ...
    if (!f)
        fail ("could not read exr from %s", filename);
    int r;
...
read_beheaded_exr (struct iio_image *x, FILE * fin, char *header, int nheader)
{
    ...
    if (r)
        fail ("read whole exr returned %d", r);
                        ...
                        FILE * fin, char *header, int nheader)
{
    ...
    sprintf (command, "/usr/bin/convert - %s < %s", ppmname, filename);
...
iio_save_image_as_png (const char *filename, struct iio_image *x)
{
    ...
    if (!pp)
        fail ("png_create_write_struct fail");
    png_infop pi = png_create_info_struct (pp);
    if (!pi)
        fail ("png_create_info_struct fail");
    if (setjmp (png_jmpbuf (pp)))
        fail ("png write error");

    if (x->dimension != 2)
        fail ("can only save 2d images");
    int width = x->sizes[0];
...
iio_save_image_as_tiff (const char *filename, struct iio_image *x)
{
    ...
    FORI (x->sizes[1])
    {
        ...
        if (r < 0)
            fail ("error writing %dth TIFF scanline", i);
    }
...
iio_save_image_as_rim_fimage (const char *fname, struct iio_image *x)
{
    if (x->type != IIO_TYPE_FLOAT)
        fail ("fimage expects float data");
    if (x->dimension != 2)
        fail ("fimage expects 2d image");
    if (x->pixel_dimension != 1)
        fail ("fimage expects gray image");
...
iio_save_image_as_rim_cimage (const char *fname, struct iio_image *x)
{
    if (x->type != IIO_TYPE_UINT8)
        fail ("cimage expects byte data");
    if (x->dimension != 2)
        fail ("cimage expects 2d image");
    if (x->pixel_dimension != 1)
        fail ("cimage expects gray image");
    FILE *f = xfopen (fname, "w");
    fputc ('M', f);
    fputc ('I', f);
    rim_putshort (f, 2);
...
iio_save_image_default (const char *filename, struct iio_image *x)
{
    ...
    if (x->dimension != 2)
        fail ("de moment només escrivim 2D");
    ...
    if (typ != IIO_TYPE_DOUBLE && typ != IIO_TYPE_FLOAT && typ != IIO_TYPE_UINT8
            && typ != IIO_TYPE_INT16 && typ != IIO_TYPE_INT8)
    ...
    {
        int m = these_floats_are_actually_bytes (x->data,
                x->sizes[0] *
                x->sizes[1]) ? 255 : 65535;
